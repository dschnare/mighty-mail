...
var MediaObject = React.createClass({displayName: "MediaObject",
	...
	getColumns: function () {
		...
		var mediaFloat = media.props.float;
		var mediaWidth = media.props.mediaWidth + gutter * 2;
		var contentWidth = this.props.width - mediaWidth;

		delete media.props.width;
		delete media.props.float;

		var mediaColProps = this.getMediaColProps(mediaFloat, mediaWidth, gutter);
		...
		var cols = [
			content,
			React.createElement(Col, React.__spread({},  mediaColProps), media)
		];
	...
	getMediaColProps: function (mediaFloat, mediaWidth, gutter) {
		return {
			width: mediaWidth,
			float: mediaFloat,
